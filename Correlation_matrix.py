...
#Load NPP
NPP_CbPM_interp = loadmat(r'.\CbPM_interp2MHW.mat')
NPP_CbPM_interp = NPP_CbPM_interp['CbPM_interp2MHW']
NPP_CbPM_interp = NPP_CbPM_interp + mask_ts

ds = xr.open_mfdataset(r'.\Max_SSTA\*.nc', parallel=True)
...
# ax.plot([lon_point_min, lon_point_max, lon_point_max, lon_point_min, lon_point_min],
#         [lat_point_min, lat_point_min, lat_point_max, lat_point_max, lat_point_min],
...
# areas = [
#     {'lat_range': (-67, -55), 'lon_range': (-76, -50)},    #AP + Drake Passage
#     {'lat_range': (-63, -50), 'lon_range': (79, 99)},      #Davis Sea
#     {'lat_range': (-67, -60), 'lon_range': (-120, -80)},   #Amundsen-Bellingshausen
#     {'lat_range': (-63, -50), 'lon_range': (20, 60)}       #Cooperation-Cosmonauts sea
# ]
...
for area in areas:
    ...
    ax.plot(lon_mesh, lat_mesh, color='purple', linewidth=2, transform=ccrs.PlateCarree())

    

outfile = r'.\Correlation_MaxInt_NPP_regions.png'
...
Max_SSTA_global = pd.read_excel (r'.\Max_SSTA_NPP.xlsx', header=None)
...
Freq_global = pd.read_excel (r'.\MHW_Freq_NPP.xlsx', header=None)
...
Dur_global = pd.read_excel (r'.\MHW_Dur_NPP.xlsx', header=None)
...
CumInt_global = pd.read_excel (r'.\MHW_CumInt_NPP.xlsx', header=None)
...
outfile = r'.\Scatter_NPP_MHWs.png'
fig.savefig(outfile, dpi=600, bbox_inches='tight', pad_inches=0.5)
