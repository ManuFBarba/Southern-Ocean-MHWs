...
PDO = pd.read_excel (r'.\PDO_Index.xlsx', header=None)
ENSO = pd.read_excel (r'.\ENSO34_Index.xlsx', header=None)
AMO = pd.read_excel (r'.\AMO_Index.xlsx', header=None)
SAM = pd.read_excel (r'.\SAM_Index.xlsx', header=None)
AAO = pd.read_excel (r'.\AAO_Index.xlsx', header=None)
SOI = pd.read_excel (r'.\SOI.xlsx', header=None)
TSA = pd.read_excel (r'.\TSA_Index.xlsx', header=None)
IPO = pd.read_excel (r'.\TPI_IPO.xlsx', header=None)
...
axs[0, 0].plot(time, PAC_Max_SSTA_ts, '-', color='peru', linewidth=2, label='Pacific')
axs[0, 0].plot(time, ATL_Max_SSTA_ts, '-', color='red', linewidth=2, label='Atlantic')
axs[0, 0].plot(time, IND_Max_SSTA_ts, '-', color='gold', linewidth=2, label='Indian')
# axs[0, 0].plot(time, res_Max_SSTA.intercept + res_Max_SSTA.slope*time, 'k--')
ax2 = axs[0, 0].twinx()
ax2.plot(time, ENSO, 'gray', linewidth=3, alpha=1, label='ONI')
ax2.fill_between(time, ENSO-(ENSO_error), ENSO+(ENSO_error),
...
# axs[0, 0].legend(loc='upper center', bbox_to_anchor=(0.5, 1), ncol=4, frameon=False)


#Frequency
# res_cnt = stats.linregress(time, PAC_MHW_cnt)
axs[0, 1].plot(time, PAC_MHW_cnt, '-', color='peru', linewidth=2)
axs[0, 1].plot(time, ATL_MHW_cnt, '-', color='red', linewidth=2)
axs[0, 1].plot(time, IND_MHW_cnt, '-', color='gold', linewidth=2)
# axs[0, 1].plot(time, res_cnt.intercept + res_cnt.slope*time, 'k--')
ax2 = axs[0, 1].twinx()
ax2.plot(time, ENSO, 'gray', linewidth=3, alpha=1)
ax2.fill_between(time, ENSO-(ENSO_error), ENSO+(ENSO_error),
    alpha=0.5, edgecolor='gray', facecolor='gray',
    linewidth=0, antialiased=True)
ax2.tick_params(length=10, direction='in', color='gray', axis='y', colors='gray')
...
axs[1, 0].plot(time, PAC_MHW_cnt, '-', color='peru', linewidth=2)
axs[1, 0].plot(time, ATL_MHW_cnt, '-', color='red', linewidth=2)
...
axs[1, 1].plot(time, PAC_MHW_cum, '-', color='peru', linewidth=2)
axs[1, 1].plot(time, ATL_MHW_cum, '-', color='red', linewidth=2)
...
axs[2, 0].plot(time, PAC_MHW_td, '-', color='peru', linewidth=2)
axs[2, 0].plot(time, ATL_MHW_td, '-', color='red', linewidth=2)
...
axs[2, 1].plot(time, MHW_PAC_Area, '-', color='peru', linewidth=2)
axs[2, 1].plot(time, MHW_ATL_Area, '-', color='red', linewidth=2)
...
# axs[0, 0].plot(time, res_Max_SSTA.intercept + res_Max_SSTA.slope*time, 'k--')
ax2 = axs[0, 0].twinx()
ax2.plot(time, SAM, 'gray', linewidth=3, alpha=1, label='SAMI')
ax2.fill_between(time, SAM-(SAM_error), SAM+(SAM_error),
    alpha=0.5, edgecolor='gray', facecolor='gray',
    linewidth=0, antialiased=True)
ax2.tick_params(length=10, direction='in', color='gray', axis='y', colors='gray')
ax2.spines['right'].set_color('gray')
