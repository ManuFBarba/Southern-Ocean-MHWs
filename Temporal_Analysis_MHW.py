...
ds = Dataset(r'.\SST_ANT_1982-2021_40.nc', 'r')

dz = Dataset(r'.\ERA5_1982-2021_T2M_ANT.nc', 'r')
...
outfile = r'.\MHW_SouthShetlands.png'
fig.savefig(outfile, dpi=300, bbox_inches='tight', pad_inches=0.5)
outfile = r'.\MHW_SouthShetlands_hq.png'
...
#Save plots so far
outfile = r'.\MHW_DrakePassage.png'
fig.savefig(outfile, dpi=600, bbox_inches='tight', pad_inches=0.5)
...
## Fig Methods Explaining Onset, Decay, and MHW metrics ##
...
plt.plot(dates, clim['thresh'], 'g-')
plt.plot(dates, lmst, 'r-')
plt.title('(63S, 177E)', fontsize=30)
plt.xlim(ts, te)
#plt.ylim(sst.min()-0.5, sst.max()+0.5)
plt.ylim(-1, 5.3)
plt.ylabel(r'[$^\circ$C]', fontsize=30, labelpad=18)
ax.yaxis.set_label_position('right')
ax.yaxis.tick_right()
plt.tick_params(length=7, direction='out')
# Find indices for all five MHWs before and after event of interest and shade accordingly
for ev0 in np.arange(ev-5, ev+40, 1):
    # Find indices for MHW of interest (2020 event) and shade accordingly
    t1 = np.where(t==mhws['time_start'][ev0])[0][0]
    t2 = np.where(t==mhws['time_end'][ev0])[0][0]
    #if sst[t1:t2+1].any() > lmst.all():
    plt.fill_between(dates[t1-1:t2+1], sst[t1-1:t2+1], clim['thresh'][t1-1:t2+1], \
                     color='r')

leg=plt.legend(['SST', 'SSTc', '95th percentile SST', 'LMST', 'MHW event'], loc='lower right', frameon=False, fontsize=25)
...
outfile = r'C:\Users\Manuel\Desktop\Paper_SO_MHWs\Revised_Manuscript_Rejected_NatComms\Figuras\MHW_metrics.png'
